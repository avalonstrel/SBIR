...
def train():
    ...
    for epoch in range(opt.start_epoch, opt.start_epoch + opt.num_epoch):
        ...
        if epoch % 2 == 0:
            ...
            train_data_loader.query_sketch()

        else:
            opt.query_what = "image"
            train_data_loader.query_image()
        if opt.model == 'sphere_model':
            distance_fun = cosine_distance
        else:
            distance_fun = euclidean_distance
        hard_negative_mining(model.feat_extractor, train_data_loader.dataset, opt.query_what, distance_fun, sample_num=(10,50))
